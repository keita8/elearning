...
class User(AbstractBaseUser):
    ...
    admin = models.BooleanField(default=False)  # a superuser
    last_login = models.DateTimeField(
        default=timezone.now, null=True, blank=True, verbose_name="Dernière connexion"
    )
    date_joined = models.DateTimeField(
...
class TeacherProfile(models.Model):     user = models.OneToOneField(         User, on_delete=models.CASCADE, verbose_name="Adresse email"     )     lastname = models.CharField(max_length=100, verbose_name="Nom de famille")     firstname = models.CharField(max_length=100, verbose_name="Prénom")     birth_date = models.DateField(         verbose_name="Date de naissance", default=timezone.now     )     phone_number = PhoneNumberField(blank=True, null=True, verbose_name="Téléphone")     bio = tinymce_models.HTMLField(blank=True, null=True)     avatar = models.ManyToManyField(Image)
