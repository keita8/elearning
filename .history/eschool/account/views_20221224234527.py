...
@api_view(['GET'])
def all_student(request, *args, **kwargs):
    queryset = StudentProfile.objects.all()
    data = {}
    if queryset:
        data = StudentProfileSerializer(queryset, many=True).data
    return Response(data)


@api_view(["GET"])
def all_teacher(request, *args, **kwargs):
    queryset = TeacherProfile.objects.all()
    print(queryset)
    data = {}
    if queryset:
        data = TeacherProfileSerializer(queryset, many=True).data
    return Response(data)

@api_view(["GET"])
def all_user(request, *args, **kwargs):
    queryset = TeacherProfile.objects.all()
    print(queryset)
    data = {}
    if queryset:
...
def student_profile(request):
    ...
    if request.method == "POST":
        ...
        user_form = UserLoginForm(instance=request.user)
        profile_form = StudentProfileForm(instance=user_profile)
