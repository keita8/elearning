...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            if user.is_authenticated and user.user.Role == 'TEACHER':
                return redirect("accounts:student")
            elif user.is_authenticated and user.role == "TEACHER":
                return render(request, 'layouts/teacher_homepage.html', context={'user':user})
...
def teacher_homepage(request):
    if request.user.is_authenticated and teacher_required== "TEACHER":
