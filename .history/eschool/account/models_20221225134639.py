...
def upload_image_path_video(instance, filename):
    new_filename = random.randint(1, 3910207878)
    name, ext = get_filename_ext(filename)
    final_filename = f"{new_filename}{ext}"
    return f"subject/lesson/content/video/{new_filename}/{final_filename}"

def upload_image_path_pdf(instance, filename):
    new_filename = random.randint(1, 3910207878)
    name, ext = get_filename_ext(filename)
    final_filename = f"{new_filename}{ext}"
    return f"subject/lesson/content/pdf/{new_filename}/{final_filename}"

def upload_image_path_course(instance, filename):
    new_filename = random.randint(1, 3910207878)
    name, ext = get_filename_ext(filename)
    final_filename = f"{new_filename}{ext}"
    return f"course/image/{new_filename}/{final_filename}"


class UserManager(BaseUserManager):
    def _create_user(
        self, email, password, is_admin, is_staff, is_superuser, **extra_fields
    ):
        if not email:
...
class Account(AbstractBaseUser, PermissionsMixin):
    ...
    @property
    def get_user_role(self) -> str | Role:
        return self.role
...
class Student(Account):
    ...
    grade = models.ForeignKey(Subject, verbose_name="Classe", on_delete=models.CASCADE)
...
class StudentProfile(models.Model):
    ...
    lastname = models.CharField(
        max_length=100, verbose_name="Nom de famille", default=""
    )
...
def delete_teacher(sender, instance=None, **kwargs):
    try:
        instance.user
...
post_delete.connect(delete_teacher, sender=TeacherProfile)


def delete_student(sender, instance=None, **kwargs):
    try:
        instance.user
    except User.DoesNotExist:
        pass
...
post_delete.connect(delete_student, sender=StudentProfile)
